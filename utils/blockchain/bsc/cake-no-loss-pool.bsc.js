...
class BSCCakeNoLossPool extends BaseBSC {

    constructor(opts) {
        super(opts);
        this.contractAbi = require('../../../configs/bsc/cake-no-loss-pool.abi');
    ...
    async getTVLAndVolume(_price = null) {
        try {
            ...
            let _tvl = parseFloatNumber(parseInt(_result[13]) / 1e18, 18);
            _price = parseFloatNumber(parseInt(_result[10]) / 1e18, 18);
            return {
                tvl: _tvl * _price,
                volume: 0
            };
...
module.exports = BSCCakeNoLossPool;